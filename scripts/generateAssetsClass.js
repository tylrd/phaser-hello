...
.option(
  '--dev',
  'Use webpack.dev.config.js for some values, excluding this will use webpack.dist.config.js (currently only GOOGLE_WEB_FONTS is being used).'
)
.parse(process.argv);
...
if (!Object.keys(loaderTypes.image).length) {
    ...
    for (var i in loaderTypes.image) {
        ...
        for (var t in loaderTypes.image[i]) {
            shell.ShellString('\n        static get' +
                              loaderTypes.image[i][t].toUpperCase() +
                              '(): string { return require(\'assets/' +
                              i +
                              '.' +
                              loaderTypes.image[i][t] +
...
if (!Object.keys(loaderTypes.spritesheet).length) {
    ...
    for (var i in loaderTypes.spritesheet) {
        ...
        shell.ShellString('\n        static get' +
                          loaderTypes.spritesheet[i][1].toUpperCase() +
                          '(): string { return require(\'assets/' +
                          i +
                          '.' +
                          loaderTypes.spritesheet[i][0] +
                          '.' +
                          loaderTypes.spritesheet[i][1] +
                          '\'); }').toEnd(assetsClassFile);
        ...
        if (spritesheetProperties.length < 2 || spritesheetProperties.length > 5) {
            console.log('Invalid number of Spritesheet properties provided for \'' +
                        i +
                        '\'. Must have between 2 and 5; [frameWidth, frameHeight, frameMax, margin, spacing] frameWidth and frameHeight are required');
        }

        shell.ShellString('\n        static getFrameWidth(): number { return ' +
                          parseInt(spritesheetProperties[0] ? spritesheetProperties[0] : -1) +
                          '; }').toEnd(assetsClassFile);
        shell.ShellString('\n        static getFrameHeight(): number { return ' +
                          parseInt(spritesheetProperties[1] ? spritesheetProperties[1] : -1) +
                          '; }').toEnd(assetsClassFile);
        shell.ShellString('\n        static getFrameMax(): number { return ' +
                          parseInt(spritesheetProperties[2] ? spritesheetProperties[2] : -1) +
                          '; }').toEnd(assetsClassFile);
        shell.ShellString('\n        static getMargin(): number { return ' +
                          parseInt(spritesheetProperties[3] ? spritesheetProperties[3] : 0) +
                          '; }').toEnd(assetsClassFile);
        shell.ShellString('\n        static getSpacing(): number { return ' +
                          parseInt(spritesheetProperties[4] ? spritesheetProperties[4] : 0) +
                          '; }').toEnd(assetsClassFile);
...
if (!Object.keys(loaderTypes.atlas).length) {
    ...
    for (var i in loaderTypes.atlas) {
        ...
        for (var t in loaderTypes.atlas[i]) {
            var dataFile = (
              'assets/' + i + '.' + loaderTypes.atlas[i][t]
            );
            var fileData = null;
            var json = null;
            var parser = null;
                ...
                try {
                    ...
                    if (Array.isArray(json['frames'])) {
                        ...
                        for (var a in json['frames']) {
                            ...
                            shell.ShellString('\n        ' + toPascalCase(frame) + ' = <any>\'' + frameFull + '\',')
                            .toEnd(assetsClassFile);
                        }
                    } else {
                        dataTypes.push('Hash');
                ...
                try {
                    ...
                    parser.parseString(fileData.substring(0, fileData.length), function(err, result) {
                        for (var x in result['TextureAtlas']['SubTexture']) {
                            frameFull = (
                              result['TextureAtlas']['SubTexture'][x]['$']['name']
                            );
                            indexOfExtension = frameFull.lastIndexOf('.');
                            if (indexOfExtension === -1) {
                                frame = frameFull;
        ...
        shell.ShellString('\n    export class ' + toPascalCase(i) + ' {').toEnd(assetsClassFile);
        shell.ShellString('\n        static getName(): string { return \'' + i.split('/').pop() + '\'; }').toEnd(
          assetsClassFile);
        for (var e in dataExtensions) {
            shell.ShellString('\n\n        static get' +
                              dataExtensions[e].toUpperCase() +
                              dataTypes[e] +
                              '(): string { return require(\'assets/' +
                              i +
                              '.' +
...
if (!Object.keys(loaderTypes.audio).length) {
    ...
    for (var i in loaderTypes.audio) {
        ...
        for (var t in loaderTypes.audio[i]) {
            shell.ShellString('\n        static get' +
                              loaderTypes.audio[i][t].toUpperCase() +
                              '(): string { return require(\'assets/' +
                              i +
                              '.' +
                              loaderTypes.audio[i][t] +
                              '\'); }').toEnd(assetsClassFile);
        }
...
if (!Object.keys(loaderTypes.audiosprite).length) {
    ...
    for (var i in loaderTypes.audiosprite) {
        for (var t in loaderTypes.audiosprite[i]) {
            var dataFile = (
              'assets/' + i + '.' + loaderTypes.audiosprite[i][t]
            );
            var fileData = null;
            var json = null;
            var sprite = null;
            ...
            if (jsonExtensions.indexOf(loaderTypes.audiosprite[i][t]) !== -1) {
                ...
                try {
                    ...
                    for (var h in json['spritemap']) {
                        ...
                        shell.ShellString('\n        ' + toPascalCase(sprite) + ' = <any>\'' + sprite + '\',').toEnd(
                          assetsClassFile);
                    }
                } catch (e) {
        ...
        for (var t in loaderTypes.audiosprite[i]) {
            shell.ShellString('\n        static get' +
                              loaderTypes.audiosprite[i][t].toUpperCase() +
                              '(): string { return require(\'assets/' +
                              i +
                              '.' +
                              loaderTypes.audiosprite[i][t] +
                              '\'); }').toEnd(assetsClassFile);
        }
...
if (!Object.keys(loaderTypes.font).length) {
    ...
    for (var i in loaderTypes.font) {
        ...
        var cssFileData = fs.readFileSync((
                                            'assets/' + i + '.css'
                                          ), 'ascii');
        ...
        for (var t in loaderTypes.font[i]) {
            shell.ShellString('\n        static get' +
                              loaderTypes.font[i][t].toUpperCase() +
                              '(): string { return require(\'!file-loader?name=assets/fonts/[name].[ext]!assets/' +
                              i +
                              '.' +
                              loaderTypes.font[i][t] +
                              '\'); }').toEnd(assetsClassFile);
        }
...
if (!Object.keys(loaderTypes.bitmap_font).length) {
    ...
    for (var i in loaderTypes.bitmap_font) {
        ...
        for (var t in loaderTypes.bitmap_font[i]) {
            shell.ShellString('\n        static get' +
                              loaderTypes.bitmap_font[i][t].toUpperCase() +
                              '(): string { return require(\'assets/' +
                              i +
                              '.' +
                              loaderTypes.bitmap_font[i][t] +
                              '\'); }').toEnd(assetsClassFile);
        }
...
if (!Object.keys(loaderTypes.json).length) {
    ...
    for (var i in loaderTypes.json) {
        ...
        for (var t in loaderTypes.json[i]) {
            shell.ShellString('\n        static get' +
                              loaderTypes.json[i][t].toUpperCase() +
                              '(): string { return require(\'assets/' +
                              i +
                              '.' +
                              loaderTypes.json[i][t] +
                              '\'); }').toEnd(assetsClassFile);
        }
...
if (!Object.keys(loaderTypes.xml).length) {
    ...
    for (var i in loaderTypes.xml) {
        ...
        for (var t in loaderTypes.xml[i]) {
            shell.ShellString('\n        static get' +
                              loaderTypes.xml[i][t].toUpperCase() +
                              '(): string { return require(\'assets/' +
                              i +
                              '.' +
                              loaderTypes.xml[i][t] +
...
if (!Object.keys(loaderTypes.text).length) {
    ...
    for (var i in loaderTypes.text) {
        ...
        for (var t in loaderTypes.text[i]) {
            shell.ShellString('\n        static get' +
                              loaderTypes.text[i][t].toUpperCase() +
                              '(): string { return require(\'assets/' +
                              i +
                              '.' +
                              loaderTypes.text[i][t] +
                              '\'); }').toEnd(assetsClassFile);
        }
...
if (!Object.keys(loaderTypes.script).length) {
    ...
    for (var i in loaderTypes.script) {
        ...
        for (var t in loaderTypes.script[i]) {
            shell.ShellString('\n        static get' +
                              loaderTypes.script[i][t].toUpperCase() +
                              '(): string { return require(\'assets/' +
                              i +
                              '.' +
                              loaderTypes.script[i][t] +
                              '\'); }').toEnd(assetsClassFile);
        }
...
if (!Object.keys(loaderTypes.shader).length) {
    ...
    for (var i in loaderTypes.shader) {
        ...
        for (var t in loaderTypes.shader[i]) {
            shell.ShellString('\n        static get' +
                              loaderTypes.shader[i][t].toUpperCase() +
                              '(): string { return require(\'assets/' +
                              i +
                              '.' +
                              loaderTypes.shader[i][t] +
                              '\'); }').toEnd(assetsClassFile);
        }
...
if (!Object.keys(loaderTypes.misc).length) {
    ...
    for (var i in loaderTypes.misc) {
        ...
        for (var t in loaderTypes.misc[i]) {
            shell.ShellString('\n        static get' +
                              loaderTypes.misc[i][t].toUpperCase() +
                              '(): string { return require(\'assets/' +
                              i +
                              '.' +
                              loaderTypes.misc[i][t] +
                              '\'); }').toEnd(assetsClassFile);
        }
